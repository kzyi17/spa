...
//     	print_r($this->autoCharset($filename,'utf-8','gbk'));
//     	die;
    ...
    public function upload($savePath ='') {
    	...
    	foreach($files as $key => $file) {
    		...
    		if(!empty($file['name'])) {
    			...
    			if($this->autoSub) {
    				// 使用子目录保存文件
    				$savefolder = $this->getSubName(array('savename'=>$savename,'savepath'=>$file['savepath']));
    				$file['savename'] = $savefolder.$savename;
    			}else{
    				$file['savename']   = $savename;
    			}
    			
    			$file['savename'] = $file['savename'].".".$file['extension'];
    			...
    			//上传成功后保存文件信息，供其他地方调用
    			//重新调整返回信息
    			$file['fullsavename']	= $file['savename'];
    			unset($file['tmp_name'],$file['error'],$file['savepath'],$file['savename']);
    			$file['name']	= $savename;
    			$file['path']	= $savefolder;
