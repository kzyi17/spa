...
<DIV class=mainBody> <DIV id=Left>
  ...
  <DIV id=Right>
		...
		<tbody>
			<tr>
				<td>
					...
					<a href="<?php echo U('merchant/merchantAdd');?>" class="btn">添加商家</a>
				...
				<td align="right">
				<form action="<?php echo U('merchant/merchantList');?>" method="post" name="searchForm" id="quickForm">
    ...
    <table width="99.5%" border="0" cellpadding="0" cellspacing="0" class="tab">
      ...
      <tbody>
        <?php if(is_array($merchantList)): $i = 0; $__LIST__ = $merchantList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr id="<?php echo ($vo['product_id']); ?>">
            <td align="center"><input name="del_id[]" type="checkbox" value="<?php echo ($vo['merchant_id']); ?>" /></td>
            <td><?php echo ($vo['merchant_name']); ?></td>
            <td><?php echo ($vo['address']); ?></td>
            <td align="center"><?php echo ($vo['pos']); ?></td>
            <td align="center"><?php echo ($vo['phone']); ?></td>
            <td align="center"><?php echo ($vo['comment_score']); ?></td>
            <td align="center"><?php echo ($vo['comment_times']); ?></td>
            <td align="center">
			...
			<?php else: ?>
				<a href='<?php echo U("Merchant/merchantEdit","id=".$vo['merchant_id']);?>'>[编辑]</a> 
				<a href="javascript:void(0)" onclick='delModel({link:"<?php echo U("merchant/merchantDel","id=".$vo["merchant_id"]) ?>"})'>[删除]</a><?php endif; ?>
			</td>
