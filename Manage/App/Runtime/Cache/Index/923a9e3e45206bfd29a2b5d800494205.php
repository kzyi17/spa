...
<DIV class=mainBody> <DIV id=Left>
  ...
  <DIV id=Right>
		...
		<tbody>
			<tr>
				<td>
					...
					<a href="<?php echo U('Share/articleCateAdd');?>" class="btn">添加分类</a>
				</td>
				<td align="right">
    			</td>
			</tr>
    ...
    <table width="99.5%" border="0" cellpadding="0" cellspacing="0" class="tab">
      ...
      <tbody>
        <?php if(is_array($cateList)): $i = 0; $__LIST__ = $cateList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr id="<?php echo ($vo['product_id']); ?>">
            <td align="center"><input name="del_id[]" type="checkbox" value="<?php echo ($vo['cate_id']); ?>" /></td>
            <td align="center"><?php echo ($vo['cate_name']); ?></td>
            <td align="center"><?php echo ($vo['is_system']==1 ? '<span class="red">系统分类</span>':'文章分类'); ?></td>
            <td align="center">
				<a href='<?php echo U("Share/articleCateEdit","id=".$vo['cate_id']);?>'>[编辑]</a> 
				<a href="javascript:void(0)" onclick='delModel({link:"<?php echo U("Share/articleCateDel","id=".$vo["cate_id"]) ?>"})'>[删除]</a>
			</td>
