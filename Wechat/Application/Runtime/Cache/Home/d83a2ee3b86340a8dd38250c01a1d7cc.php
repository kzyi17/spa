	...
	<body>
		...
		<div class="mui-content">
		    <div id="spadetail"> 
				<ul class="mui-table-view">
					<li class="mui-table-view-cell mui-media">
						...
						<span class="mui-pull-right"><?php echo ($goodsInfo['goods_name']); ?></span>
					</li>
					<li class="mui-table-view-cell mui-media">
						<span class="mui-pull-left">消耗积分：</span>
						<span class="mui-pull-right"><?php echo ($goodsInfo['exchange_point']); ?></span>
					</li>
				</ul>
			...
			<nav class="mui-bar mui-bar-footer">
				<?php if($goodsInfo['stock'] <= 0): ?><button type="button" class="mui-btn mui-btn-warning mui-btn-block" id="btn_commit" style="min-width: 10em; max-width:80%;margin: 0 auto;" disabled="true">商品库存不够，无法兑换</button>
				<?php elseif($userInfo['sharepoint'] < $goodsInfo['exchange_point']): ?>
				...
				<?php else: ?> 
					<button type="button" class="mui-btn mui-btn-warning mui-btn-block" id="btn_commit" style="min-width: 10em; max-width:80%;margin: 0 auto;">确认兑换</button><?php endif; ?>
			</nav>
		</div>
		...
		<script type="text/javascript">
			...
			var addressJsonData = <?php echo json_encode($userInfo['addressList']);?>;
			var default_address = null;
			var choseAddressId = null;
			var goodsId = <?php echo ($goodsInfo['sharegoods_id']); ?>;
			...
			document.getElementById("btn_commit").addEventListener('tap',function(){
				...
				Api.exchangeShare(param,function(res){
					mui.alert(res.successmsg,'兑换提示','确定',function(e){
						window.location = "<?php echo U('user/exchange');?>";
					});
				},function(res){
